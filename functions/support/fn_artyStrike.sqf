...
if(!_busy || isNil "_busy") then {
	...
	if (_isInRange) then {
		...
		sleep 1.5;
		_arty sideChat "Firing for effect, three rounds, out.";
		sleep 3.5;
		...
		_caller sideChat format["%1, this is %2, adjust fire, over.", groupID (group _arty), groupID (group _caller)];
		sleep 3.5;
		_arty sideChat format["%2 this is %1, we are already servicing a request, out.", groupID (group _arty), groupID (group _caller)];
		_newartyStrike = [_caller, "artyStrike"] call BIS_fnc_addCommMenuItem;
