	...
	private["_vis","_pos"];
	{
		if (player distance _x < 100) then
		{
			...
			if(!_vis) then
			{
				...
				drawIcon3D ["\a3\ui_f\data\map\Markers\NATO\b_inf.paa",[1,1,1,1],_pos,1,1,0, "Armory Officer",0,0.04];
			};
		};
	...
	private["_vis","_pos"];
	{
		if(player distance _x < 100) then
		{
			...
			if(!_vis) then
			{
				_pos = visiblePosition _x;
				_pos set[2,(getPosATL _x select 2) + 2.2];
				drawIcon3D ["\a3\ui_f\data\map\Markers\NATO\b_inf.paa",[1,1,1,1],_pos,1,1,0, "Arrival Truck",0,0.04];
			};
		};
...
0 = [] execvm "scripts\tpw_rainfx.sqf"; 
